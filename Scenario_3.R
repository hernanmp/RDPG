  ...
  for(ind_t in 1: length(T_grid) )
  {
    ...
    for(iter  in 1:NMC)
    {
      ...
      for(t in 1:T)
      {
        ...
        if(t ==1   || t == v[2]+1 )
        {
          beta[t,,] = matrix(rnorm(3*n)   ,n,3)
          temp = drop(beta[t,,])
          temp =  temp  %*% t(temp)
          P[t,,]=  exp(temp)/(1+exp(temp)    )
          diag(P[t,,]) = 0
          
          A = matrix(rbinom(matrix(1,n,n),matrix(1,n,n),P[t,,]),n,n )
          aux = drop(A )
          ...
          # phat =  drop(xhat[t,,])%*%t(drop(xhat[t,,]))
          #  mean((phat -P[t,,])^2)
          # mean((P[t,,])^2)
        }
        ...
        if(t==v[1] +1)
        {
          #beta[t,,1] =  rbeta(n,50,50)
          #matrix(runif(n,-0.1,0.1),n,1 )
          #temp = drop(beta[t,,])
          #temp =  temp  %*% t(temp)
          P[t,,] =  matrix(rbeta(n^2,100,100),n,n)
          #P = Pt
          #exp(temp)/(1+exp(temp)    )
          diag(P[t,,]) = 0
          #mean(P)
